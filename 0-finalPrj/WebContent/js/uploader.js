$(function () {
    ...
    $.upLoadDefaults.property = {
        ...
        sendurl: "/Web/Ajax/upload_ajax.ashx", //发送地址
        action: "UpMethod", //文件
        btntext: "浏览...", //上传按钮的文字
        swf: "/Web/style/webuploader/Uploader.swf" //SWF上传控件相对地址
    };
    //初始化上传控件
    $.fn.InitUploader = function (b) {
        var fun = function (parentObj) {
            ...
            uploader.on('error', function (type) {
                switch (type) {
                    case 'Q_EXCEED_NUM_LIMIT':
                        message("warning", "错误：上传文件数量过多！");
                        break;
                    case 'Q_EXCEED_SIZE_LIMIT':
                        message("warning", "错误：文件总大小超出限制！");
                        break;
                    case 'F_EXCEED_SIZE':
                        message("warning", "错误：文件大小超出限制！");
                        break;
                    case 'Q_TYPE_DENIED':
                        message("warning", "错误：禁止上传该类型文件！");
                        break;
                    case 'F_DUPLICATE':
                        message("warning", "错误：请勿重复上传该文件！");
                        break;
                    default:
                        message("warning", "错误代码：" + type);
                        break;
                }
            });
            ...
            uploader.on('uploadError', function (file, reason) {
                ...
                message("warning", file.name + "上传失败，错误代码：" + reason);
            });
            ...
            uploader.on('uploadSuccess', function (file, data) {
                ...
                if (data.status == '1') {
                    //如果是单文件上传，则赋值相应的表单
                    if (!p.multiple) {
                        if (p.action == "UpFileExcel") { window.location.reload(); }
                        parentObj.siblings(".upload-name").val(data.name);
                        parentObj.siblings(".upload-path").val(data.path);
                        if (p.action == "UpUserPic") {
                            if (data.thumb != "") {
                                $("img.upload-path").attr("src", data.thumb)
                            }
                            else {
                                $("img.upload-path").attr("src", data.path);
                            }
                        }
                        if (p.action == "UpSinglePic") {
                            parentObj.siblings(".upload-path-a").attr("href", data.path).children("img").attr("src", data.path);
                        }
